...
def load_pretrained(model, pretrained_model, use_gpu=True):
    ...
    # 如果是 DistributedDataParallel 模型，则获取当前模型的 state_dict
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        model_dict = model.module.state_dict()
        # 加载预训练模型文件
        model_state_dict = torch.load(pretrained_model, weights_only=False)
    else:
        # 对于单 GPU 或 CPU 模型，先获取当前模型的 state_dict
        model_dict = model.state_dict()
        if torch.cuda.is_available() and use_gpu:
            model_state_dict = torch.load(pretrained_model, weights_only=False)
        else:
            model_state_dict = torch.load(
                pretrained_model, weights_only=False, map_location="cpu"
            )
    # 过滤不存在的参数
    for name, weight in model_dict.items():
        if name in model_state_dict.keys():
            # 如果当前模型中参数的 shape 与预训练中对应的不一致，则从预训练字典中删除该键
            if list(weight.shape) != list(model_state_dict[name].shape):
                logger.warning(
                    f"{name} not used, shape {list(model_state_dict[name].shape)} "
                    f"unmatched with {list(weight.shape)} in model."
                )
                model_state_dict.pop(name, None)

    # 加载预训练权重到模型中（注意这里设置 strict=False 允许部分匹配）
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
    ...
    # # 可以添加详细的日志记录
    # logger.debug("Current model keys: {}".format(set(model.state_dict().keys())))
    # logger.debug("Pretrained model keys: {}".format(set(model_state_dict.keys())))
    # logger.debug(
    #     "Difference: {}".format(
    #         set(model_state_dict.keys()) - set(model.state_dict().keys())
    #     )
    # )
...
def load_checkpoint(
    ...
    def load_model(model_path):
        ...
        # 自动混合精度参数
        if amp_scaler is not None and os.path.exists(
            os.path.join(model_path, "scaler.pth")
        ):
            amp_scaler.load_state_dict(
                torch.load(os.path.join(model_path, "scaler.pth"))
            )
    ...
    if configs.preprocess_conf.get("use_hf_model", False):
        save_feature_method = (
            save_feature_method[:-1]
            if save_feature_method[-1] == "/"
            else save_feature_method
        )
    ...
    if resume_model is not None or (
        os.path.exists(os.path.join(last_model_dir, "model.pth"))
...
def save_checkpoint(
    ...
    with open(os.path.join(model_path, "model.state"), "w", encoding="utf-8") as f:
        ...
        data = {
            "last_epoch": epoch_id,
            "version": __version__,
            "model_conf.model": configs.model_conf.model,
