...
class MVectorPredictor:
    ...
    def predict(self, audio_data, sample_rate=16000):
        ...
        参数:
            audio_data: 输入音频数据（支持文件路径、文件对象、字节、numpy、AudioSegment）
            sample_rate: 当 audio_data 为 numpy 数组时需要指定采样率，默认 16000
        返回:
            声纹特征向量，形状为 (D,)
        """
        # 加载音频并预处理，返回 AudioSegment 对象
        input_data = self._load_audio(audio_data=audio_data, sample_rate=sample_rate)
        # input_data.samples: numpy 数组，形状 (T,), T 为音频采样长度

        # 转换为 torch tensor，并增加 batch 维度
        # 由 (T,) -> (1, T)
        input_data = torch.tensor(input_data.samples, dtype=torch.float32).unsqueeze(0)
        
        # 使用预处理器将输入数据提取成模型所需特征
        # 输出的 audio_feature 形状通常为 (1, D)
        audio_feature = self._audio_featurizer(input_data).to(self.device)
        
        # 进行模型前向传播预测，输出 shape 依旧为 (1, D)
        # 通过取 [0] 提取出单个样本的特征，最终形状为 (D,)
    ...
    def predict_batch(self, audios_data, sample_rate=16000, batch_size=32):
        ...
        参数:
            audios_data: 音频数据列表，每个数据支持文件路径、文件对象、字节、numpy、AudioSegment
            sample_rate: 当各个音频为 numpy 数组时需要指定采样率，默认 16000
            batch_size: 分批预测时的批量大小，默认 32
        返回:
            声纹特征数组，形状为 (N, D)，N 为音频个数，D 为特征维度
        """
        audios_data1 = []
        for audio_data in audios_data:
            # 加载每个音频数据，并取其采样点数组，形状为 (T_i,)
            input_data = self._load_audio(audio_data=audio_data, sample_rate=sample_rate)
            audios_data1.append(input_data.samples)
        
        # 按照数组长度降序排列，选出最长的音频，作为 padding 的基准
        batch = sorted(audios_data1, key=lambda a: a.shape[0], reverse=True)
        max_audio_length = batch[0].shape[0]  # 最大音频长度 T_max
        input_size = len(batch)  # 音频总数 N
        
        # 创建一个全 0 的 numpy 数组用作输入数据，形状为 (N, T_max)
        inputs = np.zeros((input_size, max_audio_length), dtype=np.float32)
        input_lens_ratio = []  # 用于保存每个音频的有效长度比例
        
        for x in range(input_size):
            tensor = audios_data1[x]  # 当前音频采样数据，形状 (T_x,)
            seq_length = tensor.shape[0]
            # 将音频数据拷贝进零数组，对应位置填充数据，其余为 0 (padding)
